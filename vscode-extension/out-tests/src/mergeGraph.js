...
function mergeGraphs(textual, dpd, moduleToFile) {
    const stats = {
        matchedNodes: 0,
        unmatchedDpdNodes: 0,
        enrichedEdges: 0,
    ...
    for (const n of textual.nodes) {
        ...
        if (!name)
            continue;
        if (file)
            byFileQualified.set(`${file}::${qualified}`, n);
        if (!byName.has(name))
            byName.set(name, []);
        byName.get(name).push(n);
    }
    const dpdToTextual = new Map();
    ...
    for (const e of textual.edges) {
        ...
        if (!edgeBySrcTgt.has(key))
            edgeBySrcTgt.set(key, []);
    ...
    for (const de of dpd.edges) {
        ...
        if (existing.length > 0) {
            for (const e of existing) {
                const prev = typeof e.props.dpd_weight === 'number'
                    ? e.props.dpd_weight
                    : 0;
...
function enrichNodeProps(n, d) {
    n.props.dpd_kind = d.kind;
    n.props.dpd_body = d.body;
    n.props.dpd_prop = d.prop;
    if (d.sourceFile)
        n.props.dpd_file = d.sourceFile;
    if (d.modulePath)
        n.props.dpd_module = d.modulePath;
}
/** Elige el nodo textual que corresponde al nodo del dpdgraph. */
function chooseMatch(d, moduleToFile, byFileQualified, byName) {
    ...
    if (candidates.length === 1)
        return candidates[0];
    const byKind = candidates.filter((c) => kindCompatible(d.kind, String(c.props.kind ?? '')));
    if (byKind.length === 1)
        return byKind[0];
    // Sin más info, dejar sin match para no inventar aristas espurias.
    return null;
}
function kindCompatible(dpdKind, textualKind) {
    const tk = textualKind.toLowerCase();
    switch (dpdKind) {
        case 'cnst':
            return (tk === 'definition' ||
                tk === 'theorem' ||
                tk === 'lemma' ||
                tk === 'corollary' ||
                tk === 'proposition' ||
...
//# sourceMappingURL=mergeGraph.js.map