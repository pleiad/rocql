...
exports.IDENT = exports.IDENT_CONT = exports.IDENT_START = void 0;
...
exports.IDENT_START = String.raw `[_\p{L}]`;
exports.IDENT_CONT = String.raw `[_\p{L}\p{N}\p{M}']`;
exports.IDENT = `${exports.IDENT_START}${exports.IDENT_CONT}*`;
const DOTTED_IDENT = `${exports.IDENT}(?:\\.${exports.IDENT})*`;
const ENTRY_RE = new RegExp(`(?<!${exports.IDENT_CONT})` +
    '(?:(?:Local|Global|Polymorphic|Monomorphic|Program|Private)\\s+)*' +
    '(?<kw>' + ENTRY_KEYWORDS.join('|') + ')' +
    `\\s+(?<name>${exports.IDENT})`, 'u');
...
const CONSTRUCTOR_KIND = {
    Inductive: 'Constructor',
    CoInductive: 'Constructor',
    Record: 'Constructor',
};
// Clasificación de comandos top-level (un comando = head hasta el primer `.`).
const MODULE_DECL_RE = new RegExp(`^\\s*(?:Local\\s+|Global\\s+)?Module(\\s+Type)?\\s+(${exports.IDENT})(\\s*\\.|\\s+(?::|<:|:=|\\())`, 'u');
const SECTION_BEGIN_RE = new RegExp(`^\\s*Section\\s+(${exports.IDENT})\\s*\\.`, 'u');
const END_RE = new RegExp(`^\\s*End\\s+(${exports.IDENT})\\s*\\.`, 'u');
...
const REQUIRE_DIRECTIVE_RE = new RegExp(`^\\s*(?:From\\s+${DOTTED_IDENT}\\s+)?Require(?:\\s+(?:Import|Export))?\\s+` +
    MODULE_LIST +
    `\\.`, 'u');
const IMPORT_DIRECTIVE_RE = new RegExp(`^\\s*(?:Import|Export)\\s+` + MODULE_LIST + `\\.`, 'u');
    ...
    while (k < n) {
        ...
        if (c === '.' && (k + 1 >= n || /\s/.test(text[k + 1]))) {
            ...
            while (back > 0 && /\s/.test(text[back - 1]))
                back -= 1;
            const endWord = back;
            while (back > 0 && /[A-Za-z0-9_]/.test(text[back - 1]))
                back -= 1;
...
function extractInductiveConstructors(head) {
    ...
    for (const raw of splitTopLevel(body, '|')) {
        ...
        let ctype = '';
        if (rest.startsWith(':'))
            ctype = rest.slice(1).trim();
        out.push({ role: 'constructor', name: cname, type: ctype });
    }
    return out;
}
function extractRecordFields(head) {
    ...
    if (m) {
        ...
        if (!m2)
            return { constructor: null, fields: [] };
...
function collectIdents(text) {
    ...
    for (const match of text.matchAll(QUALIFIED_IDENT_RE)) {
        ...
        if (!w.includes('.')) {
            ...
            const segs = w.split('.');
            if (COQ_RESERVED.has(segs[0]))
                continue;
            if (COQ_RESERVED.has(segs[segs.length - 1]))
                continue;
            out.add(w);
        }
    }
...
function charToByteTable(text) {
    const tab = new Array(text.length + 1);
    let n = 0;
...
function classifyCommand(head) {
    ...
    const aliasMatch = /^\s*(?:Local\s+|Global\s+)?Module\s+([A-Za-z_][A-Za-z0-9_']*)\s*:=\s*([A-Za-z_][A-Za-z0-9_']*(?:\.[A-Za-z_][A-Za-z0-9_']*)*)/.exec(head);
    if (aliasMatch) {
        return {
            kind: 'module_alias',
            name: aliasMatch[1],
    ...
    if (secMatch)
        return { kind: 'section_begin', name: secMatch[1] };
    // End Foo.
    ...
    while (i < n) {
        ...
        if (cls.kind === 'module_begin') {
            ...
            if (cls.kw === 'Inductive' || cls.kw === 'CoInductive') {
                ...
                const kids = [];
                if (constructor)
                    kids.push({ role: 'constructor', name: constructor, type: '' });
                kids.push(...fields);
                entry.children = kids;
            }
            entries.push(entry);
...
function makeScope(name, qualifiedPath, parent) {
    return {
        name,
        qualifiedPath,
        parent,
...
function lookupSimple(stack, name) {
...
function lookupQualified(stack, parts, fileScopeForModule, resolveAlias, activeFileScope) {
    if (parts.length < 2) {
    ...
    if (expanded !== parts) {
        return lookupQualified(stack, expanded, fileScopeForModule, resolveAlias, activeFileScope);
    }
    return undefined;
}
function walkInto(scope, rest) {
    if (rest.length === 0)
        return undefined;
    let cur = scope;
...
function findSubmodule(scope, path) {
    let cur = scope;
    for (const p of path) {
...
function buildGraph(files, opts) {
    ...
    if (opts?.moduleToFile) {
        for (const [k, v] of opts.moduleToFile)
            moduleToFile.set(k, v);
    }
    for (const f of files) {
        const stem = basenameStem(f.relpath);
        if (!moduleToFile.has(stem))
            moduleToFile.set(stem, f.relpath);
    }
    /**
     ...
     */
    function resolveModuleToFile(dotted) {
        if (moduleToFile.has(dotted))
            return moduleToFile.get(dotted);
        const parts = dotted.split('.');
        for (let i = 1; i < parts.length; i += 1) {
            const suffix = parts.slice(i).join('.');
            if (moduleToFile.has(suffix))
                return moduleToFile.get(suffix);
        }
        return undefined;
    }
    const nodeId = (file, qualified) => `${file}::${qualified}`;
    ...
    for (const [filename, data] of fileData) {
        for (const e of data.entries) {
            ...
            e.id = nid;
            const hasAdmit = e.terminator === 'Admitted' ||
                (e.proof !== '' && ADMIT_TACTIC_RE.test(e.proof));
            const labels = [e.kind];
            if (hasAdmit)
                labels.push('Admitted');
            nodes.push({
                id: nid,
                labels,
                props: {
            ...
            for (const child of e.children) {
                ...
                const childKind = child.role === 'field'
                    ? 'Field'
    ...
    function fileScopeForModule(modName) {
    ...
    for (const [filename, data] of fileData) {
        ...
        for (const e of data.entries) {
            ...
            for (const child of e.children) {
                target.defs.set(child.name, nodeId(filename, e.module_path ? `${e.module_path}.${child.name}` : child.name));
            }
        }
        ...
        for (const [aliasQualified, targetPath] of data.aliases) {
            ...
            const parent = aliasParts.length > 1
    ...
    function importClosure(filename) {
        const out = [];
    ...
    const edges = [];
    let edgeCounter = 0;
    function addEdge(src, tgt, label, where) {
        if (src === tgt)
            return;
    ...
    for (const [filename, data] of fileData) {
        ...
        function shadowAdd(scope, name, id) {
        ...
        function syncStack(targetPath) {
            ...
            while (moduleStack.length < targetPath.length) {
                ...
                if (!sub) {
                    sub = makeScope(next, moduleStack.join('.'), top);
        ...
        const sortedEntries = [...data.entries].sort((a, b) => a.cmd_start_char - b.cmd_start_char);
        for (const e of sortedEntries) {
            // Aplicar inline imports que ocurrieron antes de este entry.
            while (inlineImportIdx < inlineImports.length &&
            ...
            for (const child of e.children) {
                ...
                for (const ident of collectIdents(child.type)) {
                    ...
                    if (parts.length === 1) {
                        if (ownNames.has(ident))
                            continue;
                        const tgt = lookupSimple(stack, ident);
                        if (tgt)
                            addEdge(cid, tgt, 'REFERENCES_IN_TYPE', 'type');
                    }
                    else {
                        const tgt = lookupQualified(stack, parts, fileScopeForModule, resolveAlias, fileScope);
                        if (tgt)
                            addEdge(cid, tgt, 'REFERENCES_IN_TYPE', 'type');
                    }
                }
...
function findOrCreateSubmodule(root, path) {
    let cur = root;
    const acc = [];
    for (const p of path) {
        acc.push(p);
