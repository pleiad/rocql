...
import { Connection } from 'frogql';
import { BuiltGraph, GraphEdge, GraphNode } from './buildGraph';

export function snapshotKey(file: string, qname: string): string {
  return `${file}::${qname}`;
}
 ...
 */
export function refKindOf(
  props: Record<string, unknown> | undefined,
): string {
  if (!props) return '';
  const rk = props.ref_kind as string | undefined;
  if (rk) return rk;
  const w = props.where as string | undefined;
  return w ?? '';
}

/** Clave estable de una arista para identidad ((src, tgt, label, ref_kind)). */
export function edgeIdentityKey(e: GraphEdge): string {
...
export function nodeIdentityKey(n: GraphNode): string {
 ...
 */
export function hydrateBuiltFromGdb(conn: Connection): BuiltGraph {
  const nodes: GraphNode[] = [];
  const edges: GraphEdge[] = [];

  const nodeRows = conn.execute('(n)', 1_000_000) as unknown[];
  for (const row of nodeRows) {
    const path = (row as { _paths?: Array<Array<unknown>> })._paths?.[0];
    if (!Array.isArray(path) || path.length === 0) continue;
    const n = path[0] as
      | { kind: string; labels: string[]; props: Record<string, unknown> }
      | undefined;
  ...
  const edgeRows = conn.execute('()-[e]->()', 1_000_000) as unknown[];
  let counter = 0;
  for (const row of edgeRows) {
    ...
    const src = path[0] as
      ...
      | undefined;
    const edge = path[1] as
      | { kind: string; labels: string[]; props?: Record<string, unknown> }
      | undefined;
    const tgt = path[2] as
      | { kind: string; props: Record<string, unknown> }
      | undefined;
    if (!src || !edge || !tgt) continue;
    const srcKey = snapshotKey(
      String(src.props.file ?? ''),
      String(src.props.qualified_name ?? ''),
    );
    const tgtKey = snapshotKey(
      String(tgt.props.file ?? ''),
      String(tgt.props.qualified_name ?? ''),
    );
    counter += 1;
    edges.push({
      id: `e${counter}`,
...
function edgePropsNormalized(
  props: Record<string, unknown> | undefined,
): Record<string, unknown> {
  if (!props) return {};
  const out: Record<string, unknown> = { ...props };
  const rk = refKindOf(props);
  if (rk && out.ref_kind === undefined) out.ref_kind = rk;
  delete out.where;
  return out;
}
 ...
 *     prop forward.
 */
export function preserveEnrichmentBuilt(
  textual: BuiltGraph,
  prev: BuiltGraph,
): BuiltGraph {
  const prevNodeByKey = new Map<string, GraphNode>();
  for (const n of prev.nodes) prevNodeByKey.set(nodeIdentityKey(n), n);
  const prevEdgeByKey = new Map<string, GraphEdge>();
  for (const e of prev.edges) prevEdgeByKey.set(edgeIdentityKey(e), e);

  const nodes: GraphNode[] = textual.nodes.map((n) => {
    const key = nodeIdentityKey(n);
    const prevN = prevNodeByKey.get(key);
    if (!prevN) return n;
    const merged: Record<string, unknown> = { ...n.props };
    for (const pk of Object.keys(prevN.props)) {
      if (pk.startsWith('dpd_')) merged[pk] = prevN.props[pk];
    }
  ...
  for (const e of textual.edges) {
    ...
    if (prevE && (prevE.props as Record<string, unknown>).dpd_weight !== undefined) {
      edges.push({
        ...e,
        props: {
          ...(e.props ?? {}),
          dpd_weight: (prevE.props as Record<string, unknown>).dpd_weight,
        },
      });
    } else {
      edges.push(e);
  ...
  for (const e of prev.edges) {
    if (e.labels[0] !== 'REFERENCES_ELABORATED') continue;
    const key = edgeIdentityKey(e);
 ...
 */
export function builtGraphFingerprint(g: BuiltGraph): string {
  const nodes = g.nodes
